...
steps/train_mono.sh --cmd "$train_cmd" --nj $(nproc) \
  data/train data/lang exp/mono || exit 1;

# Monophone decoding
utils/mkgraph.sh data/lang_test exp/mono exp/mono/graph || exit 1;
steps/decode.sh --cmd "$decode_cmd" --config conf/decode.config --nj $(nproc) \
  exp/mono/graph data/dev exp/mono/decode_dev
steps/decode.sh --cmd "$decode_cmd" --config conf/decode.config --nj $(nproc) \
  exp/mono/graph data/test exp/mono/decode_test

# Get alignments from monophone system.
steps/align_si.sh --cmd "$train_cmd" --nj $(nproc) \
...
steps/decode.sh --cmd "$decode_cmd" --nj $(nproc) --config conf/decode.config \
...
steps/align_fmllr.sh --cmd "$train_cmd" --nj $(nproc) \
...
steps/decode_fmllr.sh --cmd "$decode_cmd" --nj $(nproc) --config conf/decode.config \
